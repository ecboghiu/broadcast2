...
load('up_to_37_before_memory_err.mat');
nr_ineqs = ineq_nr;
for ineq_nr=1:nr_ineqs
    ...
    while meta_iteration < MAX_ITER_OUTER_LOOP
        ...
        while ( iteration<MAX_ITER_INNER_LOOP )% The following commented out coode is to modify the loop
            fprintf("\nItertaion: %d out of %d. Ineq nr: %d \n", iteration, MAX_ITER_INNER_LOOP, ineq_nr);
 ...
 %               abs(alpha-1)<ALHPA_TOL_DIST_TO_POINT) )
            ...
            fprintf("class=%d, localboundBroadcast=%f localboundNS2=%f quantumbound=%f\n", ineq_nr, localboundBroadcast, localboundNS2, quantumbound);
                        
            VIS = 0.69;
            [POVMs,finalObj,channel] = SeeSawOverAllParties(bellcoeffs, NoisyWernerState(1-VIS), POVMs, channel);
            ...
            if finalObj-localboundNS2>1e-6
               fprintf("\n\n\nFound what we're looking for. VIS= %f Bell value: %f NS2 Bound: %f\n\n\n\n", VIS, finalObj, localboundNS2);
