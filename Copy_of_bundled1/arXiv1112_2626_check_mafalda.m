    ...
    while iteration_big_loop <= MAX_ITER_BIG_LOOP 
        ...
        if alpha_iteration > ALPHA_INI_ITER
           ...
           break;
            ...
            p_entangled = ProbMultidimArray(final_state(NoisyPartiallyEntangled(0, CONST_CHI), channel), POVMs);
            p_uniform   = ProbMultidimArray(final_state(NoisyPartiallyEntangled(1, CONST_CHI), channel), POVMs);
            ineq_visibility = visibilityOfBellInequality(bellcoeffs, localboundNS2, p_entangled, p_uniform);
            fprintf("State visibility converged to %f. Ineq. visibility with no state noise: %f (max quantum: %f). Starting a new iteration.\n\n", state_visibility, ineq_visibility, quantumbound);
        end
