a = sdpvar(4,4,'hermitian','complex'); 
b = sdpvar(4,4,'hermitian','complex');
c = [0.6670    0.0303    0.0093    0.6296
     0.0413    0.4547    0.8385    0.3747
     0.6604    0.6081    0.1001    0.0718
     0.1505    0.5065    0.7344    0.6242]; % some rand(4,4) output
 c = (c+c')/2; % make real eigenvalues
 
obj = trace(Tensor(a,c,c))+trace(c);
constraints = [a>=0,b>=0, a+b >= 2*c, a+b <= 10*eye(4)];
opts = sdpsettings('solver','mosek');
optimize(constraints, obj, opts)
disp(value(obj))  % I get 4.3646
...
a = sdpvar(4,4,'hermitian','complex'); 
b = sdpvar(4,4,'hermitian','complex');
c = sdpvar(4,4,'full','real');

obj = real(trace(Tensor(a,c,c))+trace(c));
%obj = PartialTrace(c,2,[2,2]);
constraints = [a>=0, b>=0 a+b >= 2*c, a+b <= 10*eye(4)];
opts = sdpsettings('solver','mosek','verbose',2);
params_in = {c};
sols_out = {a,obj};
precomp = optimizer(constraints, obj, opts, params_in, sols_out)
 
c_ref = [0.6670    0.0303    0.0093    0.6296
         0.0413    0.4547    0.8385    0.3747
         0.6604    0.6081    0.1001    0.0718
         0.1505    0.5065    0.7344    0.6242]; % some rand(4,4) output
