...
tic
chivalues = [0:0.01:1.7];
chiresults = cell(length(chivalues),6);
for chi_idx = 1:length(chivalues)
for ineq_nr=16:16%save%NR_OF_INEQS
    ...
    TOL_DIST_TO_POINT = 1e-6; % Util for excluding "bad" points such 
...
fprintf("\n\n\nCHI LOOP chi pcrit pthresh = %f %f %f\n\n\n", CONST_CHI, value_max(1), UNSTEERABILITY_THRESHOLD);
chiresults{chi_idx, 1} = CONST_CHI;
chiresults{chi_idx, 2} = value_max(1);
chiresults{chi_idx, 3} = UNSTEERABILITY_THRESHOLD;
chiresults{chi_idx, 4} = best_visibility;
chiresults{chi_idx, 5} = best_channel;
chiresults{chi_idx, 6} = best_POVMS;
end
toc
