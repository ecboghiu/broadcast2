...
for ineq_nr=16:16%NR_OF_INEQS
    ...
    MAX_ITER_BIG_LOOP = 1; % 4How many times to try the whole process
    ...
    TOL_DIST_TO_POINT = 1e-8; % Util for excluding "bad" points such 
                                    % as visibility=0 or visibility=1 (if 
                                    % its zero, then most likely its an infeasibility)
    ALPHA_CONVERGENCE_THRESHOLD = 1e-10;
    VISIBILITY_CONVERGENCE_THRESHOLD = 1e-10;
    ...
    while iteration_big_loop <= MAX_ITER_BIG_LOOP 
        ...
        while (abs(alpha-0)<TOL_DIST_TO_POINT || abs(alpha-1)<TOL_DIST_TO_POINT) && (alpha_iteration <= ALPHA_INI_ITER)
            ...
            if LPstatus == 0
                ...
                if abs(alpha-0)> TOL_DIST_TO_POINT && abs(alpha-1)> TOL_DIST_TO_POINT
...
% % %                 end
        ...
        if state_visibility > best_visibility
            ...
            elseif strcmp(STATE_SETTINGS.name, 'pent_povm')
                if STATE_SETTINGS.IDENTITY_PLACEMENT == 'B'
                    fprintf("\n\tNew best visibility found! p=%f (UNS A->B, Steerable B->A for p>=%f)\n", state_visibility, UNSTEERABILITY_THRESHOLD);
                elseif STATE_SETTINGS.IDENTITY_PLACEMENT == 'A'
                    fprintf("\n\tNew best visibility found! p=%f (UNS B->A, Steerable A->B for p>=%f)\n", state_visibility, UNSTEERABILITY_THRESHOLD);
            ...
            p_entangled = ProbMultidimArray(final_state(NoisyState(0, STATE_SETTINGS), best_channel), best_POVMS, ins, outs);
            p_uniform   = ProbMultidimArray(final_state(NoisyState(1, STATE_SETTINGS), best_channel), best_POVMS, ins, outs);
            [p_crit, LPstatus]= visibilityOfBellInequality(bellcoeffs, localboundNS2, p_entangled, p_uniform);
            b1 = bellcoeffs.*p_entangled; b1 = sum(b1(:));
            b2 = bellcoeffs.*p_uniform; b2 = sum(b2(:));
            fprintf("Bell·p(p=1)=%g Bell·p(p=1)=%g NS2bound=%f\n", b1, b2 , localboundNS2);
