...
while meta_iteration < MAX_ITER_META && latest_alpha_meta < 1-0.6
    ...
    while abs(alpha-0)<ALHPA_TOL_DIST_TO_1 || LPstatus~=0
        ...
        p_entangled = ProbMultidimArray(final_state(NoisyWernerState(0), iniChannel), iniPovms, ins, outs);
        p_uniform   = ProbMultidimArray(final_state(NoisyWernerState(1), iniChannel), iniPovms, ins, outs);
        ...
        evaluate_bell_ineq(bellcoeffs, 0, final_state(NoisyWernerState(0), iniChannel), iniPovms, ins, outs), ...
        evaluate_bell_ineq(bellcoeffs, 0, final_state(NoisyWernerState(alpha), iniChannel), iniPovms, ins, outs), ...
        evaluate_bell_ineq(bellcoeffs, 0, final_state(NoisyWernerState(1), iniChannel), iniPovms, ins, outs),...
    ...
    while (AbsDeltaAlpha>ALPHA_CONVERGENCE_THRESHOLD && ...
        ...
        evaluate_bell_ineq(bellcoeffs, 0, final_state(NoisyWernerState(0), channel), POVMs, ins, outs), ...
        evaluate_bell_ineq(bellcoeffs, 0, final_state(NoisyWernerState(alpha), channel), POVMs, ins, outs), ...
        evaluate_bell_ineq(bellcoeffs, 0, final_state(NoisyWernerState(1), channel), POVMs, ins, outs),...
        sum(bellcoeffs .* (p_entangled),'all'), ...
        sum(bellcoeffs .* (p_uniform),'all'), ...
        sum(bellcoeffs .* (p_entangled-p_uniform),'all'));
        localbound = ClassicalOptInequality_fromLPBroadcast(bellcoeffs);
