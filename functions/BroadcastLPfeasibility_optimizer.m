function optim_object = BroadcastLPfeasibility_optimizer(nr_inputs_per_party, nr_outputs_per_party)
    ...
    params_in = {prob};
    sols_out = {qarray};
    opts = sdpsettings('solver','mosek','verbose',0,'showprogress',0, 'debug', 0, 'warning',0);
    
    %optsol = optimize(constraints, objective, ...
    %    sdpsettings('solver','mosek','verbose',0,'showprogress',0, 'debug', 0, 'warning',0));
    optim_object = optimizer(constraints, 1.0, opts, params_in, sols_out);
